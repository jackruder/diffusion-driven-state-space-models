...
| `gt_latent` | `(B, d, T)` | only when `expose_gt_latents=True` and the mode supports it; *model-latent-space* (dim d) |
| `clean_data` | `(B, D, T)` | only when `expose_clean_data=True` and the mode supports it; *observation-space* noise-free signal before observation noise is added. Currently: `lorenz`. Distinct from `gt_latent` (different space, different semantics). |
