...
The combiner composes a :class:`~ddssm.aggregators.BaseHistoryAggregator`
(which mixes the ``j``-step history into one feature) with a
:class:`~ddssm.fusions.BaseEncoderFusion` (which combines that with
``h_fut``). Use :class:`~ddssm.aggregators.IdentityAggregator` when
...
from .aggregators import BaseHistoryAggregator
from .fusions import BaseEncoderFusion
...
class CompoundCombiner(BaseEncoderCombiner):
    ...
    :class:`~ddssm.aggregators.IdentityAggregator` for ``j=1`` (no history
