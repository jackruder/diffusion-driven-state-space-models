...
_lr = SweepSpace(target=Hparams, prefix="experiment.model.config.training")
...
_full = SweepSpace(target=Hparams, prefix="experiment.model.config.training")
...
# dec_lr is TIED to enc_lr (fires as `experiment.model.config.training.dec_lr=${...enc_lr}`
...
_arch_lr_lambda_params = {
    ...
    "experiment.model.config.training.enc_lr": "tag(log, interval(5e-4, 2e-2))",
    "experiment.model.config.training.trans_lr": "tag(log, interval(5e-4, 2e-2))",
    ...
    "experiment.model.config.training.dec_lr": r"${experiment.model.config.training.enc_lr}",
    # Lambda ramp — rescaled from (2000, 8000) to keep the same
    # 10–50 % fraction of the new 3000-step budget.
    "experiment.model.config.training.lambda_ramp.start": "tag(log, interval(1e-7, 1e-3))",
    "experiment.model.config.training.lambda_ramp.steps": "range(300, 1500)",
