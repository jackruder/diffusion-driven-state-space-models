...
def objective_factory(args):
    def objective(trial: optuna.Trial) -> float:
        ...
        lambda_warmup_steps = trial.suggest_int(
            "hyperparams.lambda_warmup_steps", 0, 2000
        )
        lambda_end = trial.suggest_float("hyperparams.lambda_end", 0.3, 2)
        vae_lr = trial.suggest_float("hyperparams.vae_lr", 5e-5, 1e-2, log=True)
        trans_lr = trial.suggest_float("hyperparams.trans_lr", 5e-5, 1e-2, log=True)
        S = 1
        batch_size = 256
        overrides = [
        ...
        if is_diffusion:
            S_k = trial.suggest_int("transition.schedule.S_k", 1, 8)
