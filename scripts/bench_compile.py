...
def _worker(preset: str, steps: int, tag: str, extra: list[str]) -> None:
    ...
    with initialize_config_dir(version_base=None, config_dir=str(REPO / "src/ddssm/conf")):
        cfg = compose(
            config_name="config",
            overrides=[
                f"experiment={preset}",
    ...
    tk = dict(
        ...
        csv_log_path="/tmp/bench_metrics.csv", tensorboard_dir="/tmp/bench_tb",
        wandb_config=None,
    )
    ...
    trainer.fit(
        ...
        batch_transform=exp.data.batch_transform, **exp.training.fit_kwargs(),
    )
    torch.cuda.synchronize()
    ...
    print("BENCHJSON " + json.dumps({
        "tag": tag, "preset": preset, "nparams": nparams,
        "total_fit_s": total,
        "intervals_ms": [round(x * 1000, 3) for x in intervals],
    }))


def _run_worker(preset: str, steps: int, tag: str, compile_on: bool, extra: list[str]) -> dict:
    ...
    raise RuntimeError(f"{tag} worker produced no result.\nSTDERR tail:\n" + "\n".join(
        out.stderr.splitlines()[-15:]))
...
def main() -> None:
    ...
    overhead = (sum(max(0.0, x - cs) for x in ci) - sum(max(0.0, x - es) for x in ei)) / 1000
    speedup = es / cs
    ...
    print(f"  compiled steady = {cs:8.2f} ms/step   (step0 {ci[0]:.0f} ms  <- compile spike)")
    print(f"  speedup         = {speedup:.2f}x")
    print(f"  compile overhead= ~{overhead:.1f} s (one-time)")
    print("  break-even      = " + (f"~{be:.0f} steps" if be else "never (compiled slower on this config)"))
