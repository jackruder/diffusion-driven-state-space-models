...
def _build_pair(num_steps: int = 16):
    ...
    cfg_esm = DiffusionV2ScheduleConfig(
        ...
        k_sampling_mode="uniform", objective="esm",
    )
    cfg_dsm = DiffusionV2ScheduleConfig(
        S_k=1, k_chunk=1, num_steps=num_steps,
        k_sampling_mode="uniform", objective="dsm",
    )
...
def test_esm_dsm_both_finite_and_dsm_geq_esm(fixed_batch):
    ...
    *gradient* (Vincent 2011) but the loss values differ by a constant equal
    ...
    N = 200
    losses_esm = compute_per_sample_loss(t_esm, fixed_batch, N, seed=1000, resample_zs=True)
    losses_dsm = compute_per_sample_loss(t_dsm, fixed_batch, N, seed=2000, resample_zs=True)
...
def test_esm_variance_lower_than_dsm(fixed_batch):
    ...
    t_esm, t_dsm = _build_pair(num_steps=16)
    N = 400
    losses_esm = compute_per_sample_loss(t_esm, fixed_batch, N, seed=10, resample_zs=True)
    losses_dsm = compute_per_sample_loss(t_dsm, fixed_batch, N, seed=20, resample_zs=True)
...
def test_esm_dsm_agree_in_degenerate_limit(transition, fixed_batch):
    ...
    cfg_esm = DiffusionV2ScheduleConfig(
        S_k=1, k_chunk=1, num_steps=16, objective="esm"
    )
    cfg_dsm = DiffusionV2ScheduleConfig(
        S_k=1, k_chunk=1, num_steps=16, objective="dsm"
    )
    torch.manual_seed(0)
    t_esm = make_transition(schedule=cfg_esm)
    torch.manual_seed(0)
    ...
    out_esm = t_esm.transition_kl(
        enc_stats=enc_stats_deg, zs=zs, logq_paths=logq_paths, time_embed=time_embed,
    )
    torch.manual_seed(42)
