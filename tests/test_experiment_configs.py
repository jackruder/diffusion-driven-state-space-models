...
def test_experiment_cli_compose(name: str) -> None:
    ...
    # Each preset composes a DDSSMModelConfig via a factory wrapper that wires
    # the shared baseline / aux instances. The init-centering family uses
    # ``_build_init_centering_model``; other families (synthetic_validation,
    # gluonts_forecast, arflow_headtohead) supply their own factory. The model
    # conf is wrapped in a DDSSMAdapter; the DDSSM factory target now lives
    # under ``model.config`` (the adapter's ``config`` slot) after commit 3
    # of the model-config refactor.
    target = cfg.experiment.model.config._target_
...
def test_default_experiment_is_init_smoke_simple() -> None:
    ...
    assert cfg.experiment.model.config._target_.endswith("_build_init_centering_model")
    assert cfg.experiment.model.config.baseline_form == "zero"
    assert cfg.experiment.model.config.tracking_mode == "fixed"
...
def test_experiment_shape_baked_in(
    ...
    exp = _exp(name)
    assert exp.model.config.data_dim == expected_data_dim
    assert exp.model.config.latent_dim == expected_latent
