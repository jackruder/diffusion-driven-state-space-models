...
def test_valid_field_builds_full_path() -> None:
    ...
    params = s.params()
    assert params["experiment.training.stages.n_pretrain"] == "tag(log, int(interval(5, 500)))"
...
def test_moo_direction_objective_mismatch_raises() -> None:
    ...
    with pytest.raises(ValueError, match="direction has 2 entries"):
        s.build(sweeper="ddssm_optuna_moo", direction=["minimize", "minimize"],
...
def test_real_init_ablation_space_validates_against_stagesb() -> None:
    ...
    for f in ("n_pretrain", "sigma_pert", "anchor_lambda", "lambda_sigma_p",
              "base_lr", "dec_mult", "trans_mult",
              "stage_1_warmup_frac", "stage_2_warmup_frac"):
