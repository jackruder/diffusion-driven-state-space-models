...
def test_init_points_bake_real_dataset_and_dims() -> None:
    ...
    for p in INIT_CENTERING_STUDY.points:
        ...
        assert p.config.model.config.data_dim == data_dim
        assert p.config.model.config.latent_dim == latent
...
def test_init_variants() -> None:
    ...
    assert v["tiny"](p1d) == []
    assert v["paper"](p1d) == ["experiment.model.config.latent_dim=2"]
    assert v["paper"](pmv) == ["experiment.model.config.latent_dim=8"]
