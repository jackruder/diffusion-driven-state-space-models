...
- [ ] **Stage gradient suppression via `trainable` mask.** Per-stage gradient gating is done via `trainable.<module>=False` alone — the forward pass always reports every ELBO term, and the mask is the single mechanism that decides which submodules update. For a "recon-only" stage, freeze `trainable.transition=False`; for "trans-only", freeze the recon-side modules.
