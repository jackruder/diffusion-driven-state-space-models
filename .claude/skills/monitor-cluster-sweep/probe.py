...
def main():
    ...
    ap.add_argument(
        "--context", required=True, help="context JSON from build_context.py"
    )
    ...
    dbs = sorted(glob.glob(os.path.join(remote, "optuna", f"{prefix}_*{suffix}.db")))
    rows = []
    for db in dbs:
        ...
        for dc in derived:
            ...
            derived_out[dc["label"]] = {
                ...
                "step_p50": _q(b["steps"], 0.5),
                "step_p90": _q(b["steps"], 0.9),
            }
        ...
        rows.append({
            "cell": cell,
            "complete": cnt.get("COMPLETE", 0),
            "running": cnt.get("RUNNING", 0),
            "fail": cnt.get("FAILED", 0),
            "pruned": cnt.get("PRUNED", 0),
            "best": best,
            "derived": derived_out,
    ...
    print(
        f"{'cell':30s} {'COMP':>4s} {'RUN':>3s} {obj_hdr} {der_hdr} "
    ...
    for r in rows:
        if "error" in r:
            ...
            continue
        bs = " ".join(
            f"{(b if b is not None else float('nan')):>14.1f}" for b in r["best"]
        )
        ds = " ".join(
            (
                f"{r['derived'][d['label']]['pct']:.0f}%"
                if r["derived"][d["label"]]["pct"] is not None
                else "-"
            ).rjust(5)
            for d in derived
        )
        print(
            f"{r['cell']:30s} {r['complete']:4d} {r['running']:3d} {bs} {ds} "
            f"{(r['trial_min'] or 0):5.0f} {r['q_running']}/{r['q_pending']:<4d} "
            f"{r['q_run_timeleft_h']:5.1f} {r['proj_final'] or '-'!s:>4s}",
            file=sys.stderr,
        )
    print(
        "__JSON__"
