...
- **Stage gradient suppression.** Per-stage gradient gating is done via the `trainable` mask alone — freezing `trainable.<module>=false` is the single mechanism for excluding a submodule from a stage's optimizer step. The forward pass always computes every ELBO term.
  ...
  multi_stage:
    stages:
      - name: stage_1
        ...
        trainable: {encoder: true, decoder: true, z_init: false, transition: false, baseline: true}
      - name: stage_2
        role: joint
